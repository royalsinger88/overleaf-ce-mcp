...
def login_handler(path, ce_url=None):
    if olbrowserlogin is None:
        raise click.ClickException(
            "当前环境缺少图形登录依赖（PySide6/QtWebEngine/libEGL）。"
            "请在有桌面环境的机器执行一次 `ols login` 后拷贝 .olauth 到当前机器。"
        )
...
def sync_func(files_from, deleted_files, create_file_at_to, delete_file_at_to, create_file_at_from, from_exists_in_to,
    ...
    for name in deleted_files:
        if non_interactive:
            delete_choice = os.environ.get("OLS_DELETE_POLICY", "i").strip().lower()
            if delete_choice not in ("d", "r", "i"):
                delete_choice = "i"
        else:
